...
//    var cartVM = CartVM()
    ...
    var routeType: ProductRouteType
    ...
    var selectedProductList: [ProductModel] = []
    var taggedProductList: [ProductModel] = []
    var productsToRemove: [ProductModel] = []
    ...
    var storeProductList: [ProductModel] = []
    var selectedStoreProductList: [ProductModel] = []
    //:
    
    var pinnedProductData: ProductModel?
    ...
    var product_Callback: ((_ selectedProducts: [ProductModel]?, _ allProducts: [ProductModel])->Void)?
    //:
    
//    var savedAddressVM = SavedAddressVM()
    
    init(routeType: ProductRouteType = .present) {
...
//        }
    ...
    func fetchProductDetails(productId: String, _ completion: @escaping(_ productData: ProductModel?,  _ errorString: String?)->Void) {
        
        ProductRequest.shared.getProductDetails(productId: productId) { result in
    ...
    func fetchTaggedProducts(query: String = "", clearBeforeCall: Bool = false, _ completion: @escaping(Bool, String?)->Void) {
        ...
        ProductRequest.shared.getTaggedProductsInStream(streamId: streamId, skip: skip, limit: limit, query: query) { result in
...
//                }
    ...
    func pinProduct(productId: String, _ completion: @escaping(Bool, String?)->Void){
        ...
        ProductRequest.shared.pinProductStream(bodyData: bodyData) { result in
    ...
    func fetchPinnedProductInStream(_ completion: @escaping(_ pinnedProductId: String?, _ errorString: String?)->Void) {
        ...
        ProductRequest.shared.getPinnedProduct(streamId: streamId) { result in
    ...
    func updateOfferOnProduct(productData: ProductModel?, discountPercentage: Double, _ completion: @escaping(Bool, String?)->Void){
        ...
        ProductRequest.shared.updateProductOffer(bodyData: bodyData) { result in
...
//        }
    ...
    func updateSelectedProducts(productData: ProductModel, _ completion: @escaping(() -> Void)){
    ...
    func unselectProductFromProductList(productData: ProductModel, _ completion: @escaping(() -> Void)){
    ...
    func getProductData(productId: String) -> ProductModel? {
    ...
    func isProductExistInTaggedProductList(productData: ProductModel) -> Bool {
    ...
    func handleProductToRemove(_ productData: ProductModel){
    ...
    func addProductToRemove(_ productData: ProductModel) {
    ...
    func addProductToCart(productData: ProductModel?, buyNow: Bool = false, withLiveStreamOffer: Bool = true, _ completion: @escaping(Bool, String?)->Void) {
...
//        else { return }
        ...
        ProductRequest.shared.getAllStores(storeId: storeId, isBusiness: 1) { result in
    ...
    func fetchStoreProducts(storeId: String?, myStore: Bool = false, _ completion: @escaping(Bool, String?)->Void){
        ...
        ProductRequest.shared.fetchStoreProducts(storeId: storeId, page: page) { result in
    ...
    func updateSelectedStoreProductList(productData: ProductModel, _ completion: @escaping(() -> Void)){
...
//        }
    ...
    func createOfferAndTagProductToLiveStream(_ completion: @escaping(Bool, String?)->Void){
        ...
        ProductRequest.shared.createProductOffer(bodyData: bodyData) { result in
            let message = result.message.unwrap
            DispatchQueue.main.async {
                completion(true, nil)
            }
...
//        }
    ...
    func getProductsThatAreNotTagged(checkOffers: Bool = false) -> [ProductModel] {
        ...
        var requiredProducts: [ProductModel] = []
